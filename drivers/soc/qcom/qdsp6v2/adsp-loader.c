...
#define ADSP_SUBSYS_DOWN 0
#define ADSP_SUBSYS_LOADED 1
...
static u32 adsp_load_state = ADSP_SUBSYS_DOWN;
...
static void adsp_load_fw(struct work_struct *adsp_ldr_work)
{
	...
	if (strcmp(img_name, "adsp")) {
		dev_err(&pdev->dev, "%s: Image %s not supported\n", __func__,
				img_name);
		goto fail;
	}
...
load_adsp:
	{
		if ((adsp_load_state == ADSP_SUBSYS_DOWN) &&
		   (adsp_state == ADSP_SUBSYS_DOWN)) {
			...
			/* Update ADSP state */
			adsp_load_state = ADSP_SUBSYS_LOADED;
		} else if (adsp_load_state == ADSP_SUBSYS_LOADED) {
			dev_dbg(&pdev->dev,
			"%s: ADSP state = %x\n", __func__, adsp_load_state);
		}
...
static void adsp_loader_unload(struct platform_device *pdev)
{
	...
	if (priv->pil_h) {
		...
		/* Update ADSP state */
		adsp_load_state = ADSP_SUBSYS_DOWN;
	}
}
